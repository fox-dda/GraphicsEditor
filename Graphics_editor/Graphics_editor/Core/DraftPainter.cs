...
namespace Graphics_editor.Core
{
    /// <summary>
    ///     Художник фигур
    /// </summary>
    public class DraftPainter : IDraftPainter
    {
        ...
        ///     Фасад отрисовщиков
        /// </summary>
        private readonly IDrawerFacade _drawer;
        ...
        {
            if (State.CacheDraft is IMultipoint multipoint) multipoint.DotList.Add(clickPoint);
        }
        ...
        public void RefreshCanvas()
        {
            ...
            foreach (var draft in drawList.Where(draft => State.UndrawableDraft != draft))
            {
                _drawer.DrawShape(draft, Painter);

                if (Corrector.HighlightDraftStorage.Contains(draft)) _drawer.DrawHighlight(draft, Painter);
            }
        }
        ...
        private void MultiPointDynamicDrawing(Point mousePoint)
        {
            ...
            {
                if (State.CacheDraft is IMultipoint multipoint) multipoint.DotList[multipoint.DotList.Count - 1] = mousePoint;
            }
